...
| 3 | **Policy gate** | **Absent.** No code, no ADR. A real, working version exists in `merp-console`'s Postgres (four tiers: allow / hold / deny / ask-a-human, plus `policy_version`, `required_approvers`, `approval_ttl`) — that is a product's floor-shaped implementation, not PIL's, and the two are not the same thing yet. | — |
